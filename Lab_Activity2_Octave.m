...
disp(['Center pixel value: ', num2str(center_pixel(:,:,1)),' ',num2str(center_pixel(:,:,2)),' ',num2str(center_pixel(:,:,3))]);
...
img_red(:,:,3) = 0;
subplot(131);
imshow(img(:,:,1));title("Red Channel");
img_green = img;
img_green(:,:,1) = 0;
img_green(:,:,3) = 0;
subplot(132);
imshow(img(:,:,2));title("Green Channel");
img_blue = img;
img_blue(:,:,1) = 0;
img_blue(:,:,2) = 0;
subplot(133);
imshow(img(:,:,3));title("Blue Channel");
